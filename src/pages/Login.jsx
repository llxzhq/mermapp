...
export default function Login() {
  ...
  useEffect(() => {
    const processLogin = async () => {
      ...
      try {
        ...
        console.log("🔵 RESPUESTA MICROSOFT:", response);
        ...
        if (!idToken) throw new Error("No se recibió idToken de Microsoft");
        ...
        try {
          ...
          console.warn("⚠️ No se pudo obtener access token:", err);
        }
        ...
        console.log("STATUS BACKEND:", backendResponse.status);

        if (!backendResponse.ok) throw new Error(`Backend respondió ${backendResponse.status}`);
        ...
        if (!user.token) throw new Error("El backend no devolvió JWT");
        ...
        localStorage.setItem("ruta", String(user.ruta || ""));
        ...
        // REDIRECCIÓN
        // =========================================
        if (Number(user.rol) === 2) navigate("/home-gestion");
        else navigate("/select-branch");
      } catch (err) {
        console.error("MICROSOFT LOGIN ERROR:", err);
        triggerError(err.message || "No pudimos iniciar sesión");
        setLoading(false);
        setShowLoader(false);
      }
    };
    ...
    processLogin();
  }, [instance, accounts, navigate, setUser,]);
  ...
  const handleMicrosoftLogin = async () => {
    try {
      ...
      triggerError("No pudimos iniciar sesión con Microsoft");
      setLoading(false);
      setShowLoader(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#f6f7fb] flex flex-col justify-between">
      <div className="flex-1 flex flex-col justify-center px-6">
        <div className="flex justify-center mb-8">
          <img src={LogoMermas} className="w-48" />
        </div>
        ...
        >
          <h2 className="text-xl font-bold mb-1 text-center">Bienvenido</h2>

          <p className="text-sm text-gray-400 mb-6 text-center">Accede con tu cuenta corporativa</p>
        ...
        {showLoader && (
          ...
          >
            <div className="bg-white px-8 py-6 rounded-2xl shadow-xl flex flex-col items-center">
              ...
              <p className="text-sm font-semibold mt-4">Conectando con Microsoft...</p>
            </div>
          </motion.div>
        )}
